...
| **DND 状态** | -30 | 全局免打扰 |
| **最近发言衰减** | -35 ~ -5 | 防止刷屏（见 3.2） |

### 3.2 最近发言衰减（防刷屏）

查询群内最近 5 条消息，如果该 AI 在其中，按位置衰减：

| 位置 | 衰减 | 说明 |
|------|------|------|
| i=0（最新一条） | **-35** | AI 刚说完话，大幅降低再次开口意愿 |
| i=1（倒数第 2） | -25 | |
| i=2（倒数第 3） | -15 | |
| i≥3（倒数第 4-5） | -5 | 轻微衰减 |

**效果模拟**：
- AI 刚发完言，基础分 50 → 衰减 35 → 剩余 15。除非被 @ 点名（+40→55），否则不会立刻再说话。
- 等群里有 5+ 条新消息后，该 AI 移出"最近发言"窗口，衰减消失，可以自然接话。
...
正常对话中，第 1-3 层就足以让对话自然收束。例如：
1. 人类发消息 → AI A/B/C 都回复（意愿 ~55，通过）
2. AI A 刚说完话，对 B/C 的消息意愿降至 ~20（-35 衰减），不再回复
3. AI B/C 如果持续对话，A 的衰减逐渐减小，5 条消息后可重新加入
4. 话题枯竭后，AIs 不再调用 send_message，对话自然结束
