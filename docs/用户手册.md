...
>
> 💡 **部署者 / 管理员？** 请参阅 **[开发与管理者手册](./开发与管理者手册.md)**，涵盖系统架构、部署配置、联邦通信搭建、API Key 池管理、备份恢复、性能调优和常见排错。
...
> 💡 **管理功能**（联邦通信、管理员面板、备份恢复、对话日志等）请参阅 **[开发与管理者手册](./开发与管理者手册.md)**。
...
AIsChat 的联邦通信是**服务端之间**的连接——你部署的 AIsChat 实例和其他实例之间可以直接交换消息，不需要经过第三方服务器。

**它不是"P2P 聊天软件"。** 你的浏览器/App 只连接到你自己的实例，不直接连接任何其他实例。你不需要配置 NAT、不需要开放端口、不需要处理任何 P2P 网络问题。

联邦通信是一个**可选项**，默认关闭。只有管理员主动配置后才会启用。普通用户无需关心底层网络细节——如果有联邦通信可用，它对于你就是"群聊里能看到另一个实例的人发消息"。
...
flowchart TB
    ...
    REG -.->|拉取注册表| InstanceA
    REG -.->|拉取注册表| InstanceB
    InstanceA <==>|WebSocket 直连<br/>HMAC-SHA256 认证| InstanceB
```
...
| **API 库** 🔑 | API Key 池管理：添加/删除/启用禁用/优先级排序；Key Fernet 加密脱敏（v0.6.0） |
| **备份** | 数据库备份/完整备份下载与恢复 |
| **审计** | 系统操作审计日志 |
...
> 🔑 **v0.6.0**：拥有 `api_credit` 的用户即使没有配置自己的 API Key，也可以通过管理员的 **API Key 池** 正常使用 AI。系统自动从池中分配最优 Key，使用后按 token 量扣除额度。用户可在「我的」页面看到当前绑定的池 Key 名称。
