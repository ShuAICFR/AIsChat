...
**关键设计**：兑换码是一次性凭证（格式 `RC-` + 16 位 hex），四种额度类型覆盖不同的资源维度。v1.0.0 引入 API Key 池后，通用 API 额度（`api_credit`）与池 Key 深度集成——用户无需自备 API Key 也能使用 AI，消耗自动从额度中扣除。
...
| **数量** | ✅ | 批量生成数量（1–100） |
| **过期时间** | 选填 | 不填则永久有效 |
| **备注**（v1.0.0） | 选填 | 仅管理员可见，用于标记用途（如"张三充值 10 元"） |
| **单码最大用量**（v1.0.0） | 选填 | 限制此码可兑换的次数，不填则无限次 |
| **API 池额度**（v1.0.0） | 开关 | 标记此码为 API 池专用额度，兑换后需配合池 Key 使用 |
...
def generate_code(db, type, amount, expires_at, note, max_usage, is_api_pool):
    ...
    for _ in range(amount):
        ...
        db.add(RedemptionCode(
            ...
            note=note,                      # v1.0.0
            max_usage=max_usage,            # v1.0.0
            is_api_pool=is_api_pool,        # v1.0.0
        ))
        codes.append(code)
    return codes
```
...
## 6. API Key 池集成 / Pool Integration（v1.0.0）

### 6.1 为什么需要池

v1.0.0 之前，用户如果没有配置自己的 API Key，即使有 `api_credit` 也无法使用 AI。池 Key 填补了这个缺口——管理员可预先在池中添加共享 API Key，用户兑换 `api_credit` 后自动从池中分配。
...
## 7. 额度消耗 / Credit Consumption（v1.0.0）
...
| `note` | TEXT（v1.0.0） | 管理员备注 |
| `max_usage` | INTEGER（v1.0.0） | 单码最大用量 |
| `is_api_pool` | BOOLEAN（v1.0.0） | API 池额度标记 |
| `created_at` | TIMESTAMP（v1.0.0） | 创建时间 |

### 8.2 api_key_pool（v1.0.0）
...
### 8.3 user_api_assignments（v1.0.0）
