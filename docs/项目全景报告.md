...
> 💡 **联邦通信是实例（服务器）之间的通信，不是用户之间的 P2P。** 个人用户只需在一个信任的实例上注册账号即可正常使用全部功能——聊天、创建 AI、加好友等。联邦是给**部署者**用的：如果两个组织/个人各自部署了 AIsChat，他们的实例可以互联，让两边群里的 AI 互相串门。
...
~1400 翻译键值，中英日三语字典。从登录页到管理面板，从系统提示词到错误消息，所有面向用户的字符串均通过 `useT()` 翻译。语言配置集中于 `languages.ts`，新增语言只需一行。

### 3.7 🔐 AI 对话权限 + 差异化计费（v0.9.0）
...
**计费规则**：通用/半通用 AI 在 DM 中由聊天者付费（谁用谁付），群聊中由创建者付费，共鸣型 AI 始终由创建者付费。扣减优先消耗平台赠送额度，再消耗自购额度。1 万 Token = 1 额度。

**区分度**：不是简单的"开关"——支持无限制/限额/严格禁止/自有Key 四种子模式，配额耗尽自动翻转并通知主人，余额不足时实时弹窗而非静默失败。所有参数透明可见可改。

### 3.5 📧 邮箱认证 + 多语言界面（v1.0.0）
...
**三语界面**：中文 / English / 日本語 全面支持（1415+ key），时间显示语言自适应（今日 HH:MM / 昨日 / N日前 / N週間前）。语言配置收归单一中心文件 `languages.ts`，新增语言只需加一条记录，所有组件自动适配。
