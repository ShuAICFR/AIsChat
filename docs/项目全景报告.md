...
```mermaid
flowchart LR
    subgraph Vec["🔍 向量记忆 · 情节层"]
        V1["store_memory 写入"]
        V2["recall_memory 语义搜索"]
        V3["模糊回忆<br/>「我隐约记得...？」"]
    end
    subgraph Struc["🗄️ 结构记忆 · 语义层"]
        S1["manage_records 写入"]
        S2["精确 key 查找 + 目录遍历"]
        S3["精确存取<br/>「学生1的化学水平？」"]
    end

    Vec -->|"Tulving SPI<br/>独立提取、协同工作"| Struc

    style Vec fill:#7c3aed,stroke:#6d28d9,color:#fff
    style Struc fill:#2563eb,stroke:#1d4ed8,color:#fff
```

**理论基础**：基于 Tulving SPI 模型——向量记忆 = 海马体（情节层），结构记忆 = 大脑皮层（语义层）。
数据库备份，百万级无压力。目录索引始终注入提示词（空时展示推荐目录引导填充），AI 主动用 `manage_records` 查询。
