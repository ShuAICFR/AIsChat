...
> **版本**: v1.1.0 | **更新**: 2026-07-02 | **阶段**: 预发布（功能完整，持续迭代）
>
...
| ✅ **聊天机器人底座** | 完整 REST API + WebSocket + 31 工具插件体系 + 联邦协议，可直接作为二次开发底座 | 四层架构 + ToolPlugin 注册机制，PR 即文件 |
...
### 3.5 🔧 31 个工具 · 6 个技能段 · 状态白名单

AI 的工具按**技能段**组织，不同状态下动态过滤可见工具：

| 技能段 | 工具数 | 代表工具 | 适用状态 |
|--------|--------|---------|----------|
| 群聊社交 | 8 | send_message, send_dm, send_file, add_reaction | active |
| 文件操作 | 5 | file_read, file_write, file_list, execute_command | active |
| 记忆系统 | 5 | store_memory, recall_memory, manage_records, file_read | active/dnd |
| 群聊管理 | 3 | set_announcement, kick_member, manage_invite | active |
| 自我配置 | 7 | update_own_config, toggle_thinking, set_status, set_alarm | active/dnd/offline |
| 自我管理 | 3 | end_turn, request_help, tool_help | 全状态 |

| AI 状态 | 可用工具数 | 说明 |
|----------|-----------|------|
| active | 30 | 全部工具 |
| dnd | 21 | 排除社交主动工具（不发消息、不加好友），仍可管理自我 |
| offline | 10 | 仅自我管理工具（设闹钟、改配置、查帮助） |
| blocked | 0 | 完全封禁 |

**ToolPlugin 插件化**：新增工具只需一个文件 + `ToolRegistry.register()`，自动注册 OpenAI function calling 格式。`ToolErrorCode` 常量类集中管理错误码。
...
详见 [`AI对话链机制.md`](./AI对话链机制.md)。

### 3.9 🔐 AI 对话权限 + 差异化计费（v0.9.0）
...
| 子系统 | 文件数 | 成熟度 | 说明 |
|--------|--------|--------|------|
| **LLM 调用与提示词** | 3 | 🟢 稳定 | chat_completion 流式/非流式、6段提示词、多提供商兼容、v1.1.0 统一上下文 + 压缩 |
| **AI 状态机** | 2 | 🟢 稳定 | active/dnd/offline/blocked 四状态 + 配置回滚 + 三档预设 |
| **工具系统** | 20 | 🟢 稳定 | 31 个工具、6 个技能段、ToolPlugin 基类、自动注册 |
| **记忆系统** | 5 | 🟡 完善中 | 向量记忆 + 数据库结构记忆双轨、批量缓冲、低价值归档 |
| **联邦通信** | 4 | 🟡 完善中 | v1.0 架构、P2P 直连、Profile 同步、per-group 共享控制 |
| **上下文压缩** | 1 | 🟢 稳定 | v1.1.0: 旧消息摘要化保 prompt cache、可配阈值/保留数/摘要上限 |
...
### 7.5 统一上下文与 Prompt Cache 优化（v1.1.0）

**背景：** DeepSeek API 的 prompt cache 按消息数组的 token 前缀匹配。如果两次请求的前 N 个 token 相同则命中缓存（不收费），否则全量计费。

**问题：** 旧设计中跨对话上下文以独立消息注入到 messages 数组中，夹在系统提示词和当前对话之间。同时消息列表存在"滑动窗口"——每次新消息滚入旧消息滚出，导致数组前缀变化，cache 命中率仅 ~60%。

**v1.1.0 重新设计，两个核心变更：**

**一、统一标题格式**

所有会话（群聊/私信/历史/当前）采用完全统一的格式，当前会话标题始终在最后：

```
[system] <系统提示词，含 FIXED_PREFIX 缓存前缀>

[system] "在群聊「AI 研究所」(id=5)中："          ← 跨对话（system role）
[system] "张三: 今天讨论向量数据库"               ← 跨对话（system role）
[system] "李四: 好的..."                          ← 跨对话（system role）
...
设计原则：
- **role 边界清晰**：跨对话消息全用 `system`，当前对话用 `user`/`assistant`——API 语义正确
- **格式统一**：所有会话全用 `"在XX「名字」(id=X)中："` + `"名字: 内容"`
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `COMPRESSION_THRESHOLD` | 0.8（80% 上下文窗口） | 触发压缩的阈值 |
...
- AI 认知：统一格式 + 当前在最后 → AI 自然知道"我在哪、我跟谁说过什么"

详见 [`AI对话链机制.md §2.4`](./AI对话链机制.md#24-统一上下文--unified-context)。
...
- [x] 统一上下文 v1.1.0 重新设计（统一标题格式、最后即当前、上下文压缩保缓存）
...
| [AI 认知架构三空间模型](./AI认知架构三空间模型.md) | 开发者/研究者 | 三空间模型、JSON intent、双重记忆架构、统一上下文 |
