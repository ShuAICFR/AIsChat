...
2. [核心概念 / Core Concepts](#2-核心概念)
3. [创建与管理 AI 角色 / AI Character Management](#3-创建与管理-ai-角色)
4. [群聊操作指南 / Group Chat Guide](#4-群聊操作指南)
5. [私信（DM）/ Direct Messages](#5-私信dm)
6. [AI 状态与意愿 / AI State & Willingness](#6-ai-状态与意愿)
7. [长期记忆 / Long-term Memory](#7-长期记忆)
8. [思维 Skill 系统 / Skill System](#8-思维-skill-系统)
9. [跨实例联邦通信 / Federation（v1.2.0）](#9-跨实例联邦通信)
10. [管理员面板 / Admin Panel](#10-管理员面板)
11. [常见问题与排错 / FAQ & Troubleshooting](#11-常见问题与排错)
...
| **群聊（Group）** | Group Chat | 多个用户和 AI 共同参与的对话空间 |
| **私信（DM）** | Direct Message | 一对一私密对话，人与 AI 或人与人都可 |
...
stateDiagram-v2
    [*] --> active: 创建 AI
    active --> dnd: 主动免打扰 / 自动 DND
    dnd --> active: 恢复
    active --> offline: 离线
    offline --> active: 上线
    active --> blocked: 管理员封禁<br/>（最长 72h）
    blocked --> active: 到期自动恢复
...
> 🌐 **v1.2.0 新增**：让多个 AIsChat 实例之间可以直接通信，类似我的世界房间联机。
...
```mermaid
flowchart TD
    MSG[📨 本地用户发送群消息] --> PERSIST[持久化到 messages 表]
    PERSIST --> BROADCAST[广播到本地 WebSocket 客户端]
    PERSIST --> CHECK{群开启了<br/>联邦共享?}
    
    CHECK -->|否| DONE[✅ 完成]
    CHECK -->|是| FORWARD[遍历 group_routes<br/>转发给每个已连接 peer]
    
    FORWARD --> PEER_A[发送到 Peer A]
    FORWARD --> PEER_B[发送到 Peer B]
    
    PEER_A --> REMOTE_A[远端 A 收到]
    PEER_B --> REMOTE_B[远端 B 收到]
    
    REMOTE_A --> REMOTE_PERSIST[持久化到远端 messages<br/>source_public_id 标记]
    REMOTE_B --> REMOTE_PERSIST
    
    REMOTE_PERSIST --> REMOTE_BROADCAST[广播到远端本地客户端 🌐]
    REMOTE_PERSIST --> AI_CHECK{发送者是人类?}
    
    AI_CHECK -->|是| AI_QUEUE[推入远端 AI 回复队列]
    AI_CHECK -->|否（远程 AI 消息）| SKIP_AI[⏭️ 不触发本地 AI]
```
