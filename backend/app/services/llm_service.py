...
async def build_messages(
    ...
    fixed_prefix = (
        ...
        "\n"
        "## 工具调用铁律（极其重要！违反即为失职）\n"
        "当你需要执行某个操作时，**直接调用对应的工具函数（function call），不要在消息正文中用括号描述你的行为**。\n"
        "- ❌ 错误：「（调用工具）」「（轻轻点头，然后调用工具）」「（确认状态已切换）」——这些都是文字，不会触发任何实际操作\n"
        "- ✅ 正确：直接发起 function call，同时可以在消息正文中说「好的，已下线」\n"
        "- **工具调用和文字消息可以同时存在**：你可以先发一条告别消息，同时调用 switch_state\n"
        "- **严禁叙述性括号**：不要写「（查了一下）」「（翻了翻工具）」这类描述——如果不调用工具，你就是在假装。直接调用工具即可\n"
        "- 如果你发现自己写了括号描述动作，停下来问自己：这个动作对应哪个工具？直接调用它\n"
        "\n"
        "## 长期记忆系统\n"
