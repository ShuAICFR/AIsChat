...
| 9 | [跨实例联邦通信](#9-跨实例联邦通信) | Federation (v1.2.0) |
...
```mermaid
flowchart TD
    A[📝 注册账号<br/>Register] --> B[🔑 配置 API Key<br/>Setup Key]
    B --> C[🤖 创建 AI 角色<br/>Create Agent]
    C --> D[💬 创建群聊<br/>Create Group]
    D --> E[🎉 开始聊天<br/>Start Chat]
```
...
> 📊 **v1.2.2 新增**：管理员可查看 AI 与 LLM 的每次完整对话记录，用于调试和审计。
